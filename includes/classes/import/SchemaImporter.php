 ...
 */
class SchemaImporter extends DataImporter {
	...
	protected function read_file() {
		if ( ! Schema::is_file_valid_csv( $this->file ) ) {
	...
	protected function set_parsed_data() {
		...
		foreach ( $this->raw_data as $row_index => $row ) {
			...
			do_action( 'posterno_schema_importer_before_set_parsed_data', $row, $mapped_keys );
			...
			$this->parsed_data[] = apply_filters( 'posterno_schema_importer_parsed_data', $this->expand_data( $data ), $this );
		}
	}
	...
	public function import() {
		...
		foreach ( $this->parsed_data as $parsed_data_key => $parsed_data ) {
			...
			if ( $id ) {
				$product   = pno_get_product( $id );
				$id_exists = $product && 'importing' !== $product->get_status();
			}

			if ( $sku ) {
				$id_from_sku = pno_get_product_id_by_sku( $sku );
				$product     = $id_from_sku ? pno_get_product( $id_from_sku ) : false;
			...
			if ( $id_exists && ! $update_existing ) {
				$data['skipped'][] = new WP_Error(
					...
					esc_html__( 'A product with this ID already exists.', 'posterno' ),
					array(
						'id'  => $id,
			...
			if ( $sku_exists && ! $update_existing ) {
				$data['skipped'][] = new WP_Error(
					'posterno_schema_importer_error',
					esc_html__( 'A product with this SKU already exists.', 'posterno' ),
					array(
						'sku' => esc_attr( $sku ),
						'row' => $this->get_row_id( $parsed_data ),
					)
			...
			if ( $update_existing && ( $id || $sku ) && ! $id_exists && ! $sku_exists ) {
				$data['skipped'][] = new WP_Error(
					'posterno_schema_importer_error',
					esc_html__( 'No matching product exists to update.', 'posterno' ),
					array(
						'id'  => $id,
						'sku' => esc_attr( $sku ),
						'row' => $this->get_row_id( $parsed_data ),
