...
<form class="pno-progress-form-content posterno-importer" method="post" action="<?php echo esc_url( $this->get_next_step_link() ); ?>">
	...
	<section class="pno-importer-mapping-table-wrapper">
		<table class="widefat pno-importer-mapping-table">
			<thead>
				<tr>
					<th><?php esc_html_e( 'Column name', 'posterno' ); ?></th>
					<th><?php esc_html_e( 'Map to field', 'posterno' ); ?></th>
				</tr>
			</thead>
			<tbody>
				<?php foreach ( $headers as $index => $name ) : ?>
					...
					<tr>
						<td class="pno-importer-mapping-table-name">
							<?php echo esc_html( $name ); ?>
							<?php if ( ! empty( $sample[ $index ] ) ) : ?>
								<span class="description"><?php esc_html_e( 'Sample:', 'posterno' ); ?> <code><?php echo esc_html( $sample[ $index ] ); ?></code></span>
							<?php endif; ?>
						</td>
						<td class="pno-importer-mapping-table-field">
							...
							<select name="map_to[<?php echo esc_attr( $index ); ?>]">
								<option value=""><?php esc_html_e( 'Do not import', 'posterno' ); ?></option>
								<option value="">--------------</option>
	...
	<div class="pno-actions">
		<button type="submit" class="button button-primary button-next" value="<?php esc_attr_e( 'Run the importer', 'posterno' ); ?>" name="save_step"><?php esc_html_e( 'Run the importer', 'posterno' ); ?></button>
