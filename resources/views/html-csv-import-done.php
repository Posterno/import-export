...
<div class="pno-progress-form-content posterno-importer">
	<section class="posterno-importer-done">
		...
		if ( 0 < $imported ) {
			$results[] = sprintf(
				...
				_n( '%s item imported', '%s products imported', $imported, 'posterno' ),
		...
		if ( 0 < $updated ) {
			$results[] = sprintf(
				/* translators: %d: products count */
				_n( '%s item updated', '%s products updated', $updated, 'posterno' ),
		...
		if ( 0 < $skipped ) {
			$results[] = sprintf(
				/* translators: %d: products count */
				_n( '%s item was skipped', '%s products were skipped', $skipped, 'posterno' ),
	...
	<div class="pno-actions">
		<a class="button button-primary" href="<?php echo esc_url( admin_url( 'edit.php?post_type=product' ) ); ?>"><?php esc_html_e( 'View products', 'posterno' ); ?></a>
	</div>
</div>
